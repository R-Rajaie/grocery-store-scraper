...
url = "https://local.safeway.com/safeway.html"
driver.get(url)
...
def add_info(location):
    ...
    phone = location.find(class_="Phone-display Phone-display--withLink").get_text(strip = True)
    print(name + " " + address + " " + phone)
    all_stores.append({"Name": name, "Address": address, "Phone": phone})

states = soup.find_all(class_="Directory-listLink")
for state in states:
    driver.get("https://local.safeway.com/" + state.get('href'))
    ...
    if state.get('data-count') == '(1)':
        ...
        for city in cities:
            driver.get("https://local.safeway.com/" + city['href'])
            ...
            if city.get('data-count') == '(1)':
                ...
                for location in locations:
                    driver.get("https://local.safeway.com/" + location.find(class_="Teaser-titleLink").get('href'))
                    add_info(BeautifulSoup(driver.page_source, 'html.parser'))
...
df.to_csv("safeway_locations.csv", index=False)
driver.quit()